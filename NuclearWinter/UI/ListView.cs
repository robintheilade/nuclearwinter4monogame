...
namespace NuclearWinter.UI
{
    ...
    public class ListViewColumn
    {
        ...
        //----------------------------------------------------------------------
        public ListViewColumn(ListView listView, string text, int width, Anchor anchor)
        {
            Width = width;
            Label = new UI.Label(listView.Screen, text);
            Anchor = anchor;
        }

        public ListViewColumn(ListView listView, Texture2D texture, Color color, int width, Anchor anchor)
        {
            Width = width;
            Image = new Image(listView.Screen, texture);
            Image.Color = color;
            Anchor = anchor;
        }
    }
    ...
    abstract public class ListViewCell
    {
        ...
        //----------------------------------------------------------------------
        public ListViewCell(ListView view, string text, Texture2D image)
        {
            mListView = view;
            Text = text;
            Image = image;
        ...
        //----------------------------------------------------------------------
        protected internal abstract void DoLayout(Rectangle rectangle, int columnIndex, ListViewColumn column, ListViewRow row);
        protected internal abstract void Draw(Point location);
    }
    ...
    public class ListViewTextCell : ListViewCell
    {
        ...
        //----------------------------------------------------------------------
        public ListViewTextCell(ListView view, string text)
        : base(view, text, null)
        {
        ...
        //----------------------------------------------------------------------
        protected internal override void DoLayout(Rectangle rectangle, int columnIndex, ListViewColumn column, ListViewRow row)
        {
            int iIndicatorRight = rectangle.Right;
            if (mListView.ActionButtonsColumn == columnIndex && mListView.HoveredRow == row && mListView.ActionButtons.Count > 0)
            {
            ...
            foreach (ListViewCellIndicator indicator in Indicators)
            {
                ...
                indicator.DoLayout(new Rectangle(iIndicatorRight - miIndicatorsWidth - mListView.Style.CellHorizontalPadding - mListView.Style.ActionButtonsRightPadding, rectangle.Top + mListView.Style.IndicatorVerticalPadding, indicator.ContentWidth, mListView.Style.RowHeight - mListView.Style.IndicatorVerticalPadding * 2));
            ...
            if (mstrText != "")
            {
                ...
                while (mfTextWidth + mListView.Style.CellHorizontalPadding * 2 > column.Width - miIndicatorsWidth)
                {
                    iOffset--;
            ...
            switch (column.Anchor)
            {
                case Anchor.Start:
                    mvTextOffset.X += mListView.Style.CellHorizontalPadding;
                    break;
                case Anchor.Center:
                    mvTextOffset.X += (int)(column.Width / 2f - mfTextWidth / 2f);
                    break;
                case Anchor.End:
                    mvTextOffset.X += column.Width - mListView.Style.CellHorizontalPadding - mfTextWidth;
                    break;
            }
        }

        //----------------------------------------------------------------------
        protected internal override void Draw(Point location)
        {
            Vector2 vTextPos = new Vector2(location.X, location.Y + mListView.Style.RowHeight / 2f - (int)(mListView.Screen.Style.MediumFont.LineSpacing * 0.9f / 2f));
    ...
    public class ListViewImageCell : ListViewCell
    {
        ...
        //----------------------------------------------------------------------
        public ListViewImageCell(ListView view, Texture2D image, int forcedHeight = 0)
        : base(view, null, image)
        {
        ...
        protected internal override void DoLayout(Rectangle rectangle, int columnIndex, ListViewColumn column, ListViewRow row)
        {
            ...
            switch (column.Anchor)
            {
                case Anchor.Start:
                    ...
                    break;
                case Anchor.Center:
                    mvOffset.X += (int)(column.Width / 2f - mvSize.X / 2f);
                    break;
                case Anchor.End:
                    mvOffset.X += (int)(column.Width - mvSize.X - 10);
                    break;
            }
        }
        ...
        protected internal override void Draw(Point location)
        {
            Vector2 vImagePos = new Vector2(location.X, location.Y + mListView.Style.RowHeight / 2 - mvSize.Y / 2f);
            vImagePos += mvOffset;
    ...
    public class ListViewCellIndicator : Widget
    {
        ...
        //----------------------------------------------------------------------
        public ListViewCellIndicator(Screen screen, string text, Texture2D iconTexture = null, object tag = null)
        : base(screen)
        {
    ...
    public class ListViewRow
    {
        ...
        //----------------------------------------------------------------------
        public ListViewRow(ListViewCell[] cells, object tag)
        {
            Cells = cells;
            Tag = tag;
        }
    ...
    public class ListView : Widget
    {
        ...
        //----------------------------------------------------------------------
        public void AddColumn(string text, int width, Anchor anchor = Anchor.Center)
        {
            Columns.Add(new ListViewColumn(this, text, width, anchor));
        }

        public void AddColumn(Texture2D texture, Color color, int width, Anchor anchor = Anchor.Center)
        {
            Columns.Add(new ListViewColumn(this, texture, color, width, anchor));
        }
        ...
        public override void OnMouseMove(Point hitPoint)
        {
            if (mbIsMouseDown && FocusedRow != null)
            {
                IsDragging = DragNDropHandler != null && (
                        Math.Abs(hitPoint.Y - mMouseDownPoint.Y) > MouseDragTriggerDistance
        ...
        {
            DoScroll(-delta * 3 * (Style.RowHeight + Style.RowSpacing) / 120);
        }
        ...
        protected internal override bool OnMouseDown(Point hitPoint, int button)
        {
            ...
            else
            {
                ...
                int iFocusedRowIndex = Math.Max(0, (hitPoint.Y - (LayoutRect.Y + Padding.Top + (DisplayColumnHeaders ? Style.RowHeight : 0)) + (int)Scrollbar.LerpOffset) / (Style.RowHeight + Style.RowSpacing));
        ...
        protected internal override void OnMouseUp(Point hitPoint, int button)
        {
            ...
            if (IsDragging)
            {
                ...
                if (HitBox.Contains(hitPoint) && HoveredRow != draggedRow && DragNDropHandler != null)
                {
        ...
        //----------------------------------------------------------------------
        void SelectRowAt(Point hitPoint)
        {
            UpdateHoveredRow();
